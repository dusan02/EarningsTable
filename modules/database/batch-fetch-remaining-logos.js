...
async function batchFetchRemainingLogos() {
  console.log("=== BATCH FETCHING REMAINING LOGOS ===\n");
  ...
  // Final analysis
  console.log("\n=== FINAL ANALYSIS ===");
  const finalAnalysis = await prisma.finhubData.findMany({
    select: { symbol: true, logoUrl: true, logoSource: true },
    distinct: ["symbol"],
  });

  const totalSymbols = finalAnalysis.length;
  const withLogos = finalAnalysis.filter((s) => s.logoUrl).length;
  ...
  console.log(
    `Final logo coverage: ${Math.round((withLogos / totalSymbols) * 100)}%`
  );
}

// Run the batch processing
batchFetchRemainingLogos()
  .catch(console.error)
